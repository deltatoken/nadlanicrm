...
class CronManager
{
    ...
    public function __construct(\Nadlani\Core\Container $container)
    {
        ...
        $this->cronJobUtil = new \Nadlani\Core\Utils\Cron\Job($this->config, $this->entityManager);
        $this->cronScheduledJobUtil = new \Nadlani\Core\Utils\Cron\ScheduledJob($this->config, $this->entityManager);
    ...
    public function processPendingJobs($queue = null, $limit = null, $poolDisabled = false, $noLock = false)
    {
        ...
        foreach ($pendingJobList as $job) {
            ...
            if ($useProcessPool) {
                ...
                $job->set('pid', \Nadlani\Core\Utils\System::getPid());
            }
            ...
            if ($useProcessPool) {
                $task = new \Nadlani\Core\Utils\Cron\JobTask($job->id);
                $pool->add($task);
            } else {
                $this->runJob($job);
            }
