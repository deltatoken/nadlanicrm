...
class AclManager extends \Nadlani\Core\AclManager
{
    protected $tableClassName = '\\Nadlani\\Core\\AclPortal\\Table';

    private $mainManager = null;

    private $portal = null;

    protected $userAclClassName = '\\Nadlani\\Core\\Portal\\Acl';
    ...
    public function getImplementation($scope)
    {
        if (empty($this->implementationHashMap[$scope])) {
            ...
            $className = '\\Nadlani\\Custom\\AclPortal\\' . $normalizedName;
            if (!class_exists($className)) {
                $moduleName = $this->getMetadata()->getScopeModuleName($scope);
                if ($moduleName) {
                    $className = '\\Nadlani\\Modules\\' . $moduleName . '\\AclPortal\\' . $normalizedName;
                } else {
                    $className = '\\Nadlani\\AclPortal\\' . $normalizedName;
                }
                if (!class_exists($className)) {
                    $className = '\\Nadlani\\Core\\AclPortal\\Base';
                }
            }
