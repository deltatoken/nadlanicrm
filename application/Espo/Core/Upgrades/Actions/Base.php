...
namespace Nadlani\Core\Upgrades\Actions;

use Nadlani\Core\Utils\Util;
use Nadlani\Core\Utils\System;
...
abstract class Base
{
    ...
    public function __construct(\Nadlani\Core\Container $container, \Nadlani\Core\Upgrades\ActionManager $actionManager)
    {
        $this->container = $container;
        $this->actionManager = $actionManager;
        $this->params = $actionManager->getParams();

        $this->zipUtil = new \Nadlani\Core\Utils\File\ZipArchive($container->get('fileManager'));
    }
    ...
    /**
     * Check if version of upgrade/extension is acceptable to current version of NadlaniCrm
     *
    ...
    protected function isAcceptable()
    {
        ...
        if (isset($manifest['acceptableVersions'])) {
            $res &= $this->checkVersions($manifest['acceptableVersions'], $this->getConfig()->get('version'), 'Your NadlaniCrm version doesn\'t match for this installation package.');
        }
