...
namespace Nadlani\Core\Utils\Api;

use \Nadlani\Core\Utils\Api\Slim;
...
class Auth extends \Slim\Middleware
{
    ...
    public function __construct(\Nadlani\Core\Utils\Auth $auth, $authRequired = null, $showDialog = false)
    {
        $this->auth = $auth;
    ...
    function call()
    {
        ...
        $espoAuthorizationHeader = $request->headers('Http-Nadlani-Authorization');
        ...
        if (!isset($username) && !isset($password)) {
            $espoCgiAuth = $request->headers('Http-Nadlani-Cgi-Auth');
            if (empty($espoCgiAuth)) {
                $espoCgiAuth = $request->headers('Redirect-Http-Nadlani-Cgi-Auth');
            }
            if (!empty($espoCgiAuth)) {
