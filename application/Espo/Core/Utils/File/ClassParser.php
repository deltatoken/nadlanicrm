...
class ClassParser
{
    ...
    public function __construct(\Nadlani\Core\Utils\File\Manager $fileManager, \Nadlani\Core\Utils\Config $config, \Nadlani\Core\Utils\Metadata $metadata)
    {
    ...
    public function getData($paths, $cacheFile = false)
    {
        ...
        if ($cacheFile && file_exists($cacheFile) && $this->getConfig()->get('useCache')) {
            ...
            if ($cacheFile && $this->getConfig()->get('useCache')) {
                ...
                if ($result == false) {
                    throw new \Nadlani\Core\Exceptions\Error();
                }
            }
        }
