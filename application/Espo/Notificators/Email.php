...
namespace Nadlani\Notificators;

use \Nadlani\ORM\Entity;

class Email extends \Nadlani\Core\Notificators\Base
{
    ...
    public function process(Entity $entity, array $options = [])
    {
        ...
        if (empty($data['personEntityId'])) {
            $data['fromString'] = \Nadlani\Services\Email::parseFromName($entity->get('fromString'));
