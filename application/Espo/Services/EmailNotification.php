...
class EmailNotification extends \Nadlani\Core\Services\Base
{
    ...
    protected function getHtmlizer()
    {
        if (empty($this->htmlizer)) {
            $this->htmlizer = new \Nadlani\Core\Htmlizer\Htmlizer($this->getInjection('fileManager'), $this->getInjection('dateTime'), $this->getInjection('number'), null);
        }
    ...
    protected function getSiteUrl(\Nadlani\Entities\User $user)
    {
        if ($user->isPortal()) {
