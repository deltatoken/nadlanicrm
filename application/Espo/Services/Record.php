...
class Record extends \Nadlani\Core\Services\Base
{
    ...
    public function getListKanban($params)
    {
        ...
        $collection = new \Nadlani\ORM\EntityCollection([], $this->entityType);
        ...
        if (!empty($defs[$attribute]) && !empty($defs[$attribute]['type'])) {
            ...
            switch ($type) {
                case 'jsonObject':
                    ...
                    return \Nadlani\Core\Utils\Json::encode($value, \JSON_UNESCAPED_UNICODE);
                    break;
                case 'jsonArray':
    ...
    public function getDuplicateAttributes($id)
    {
        ...
        $fieldManager = new \Nadlani\Core\Utils\FieldManagerUtil($this->getMetadata());
