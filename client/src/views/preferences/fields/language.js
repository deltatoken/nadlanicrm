...
Nadlani.define('views/preferences/fields/language', 'views/fields/enum', function (Dep) {

    return Dep.extend({

        setupOptions: function () {
            this.params.options = Nadlani.Utils.clone(this.getConfig().get('languageList')).sort(function (v1, v2) {
                return this.getLanguage().translateOption(v1, 'language').localeCompare(this.getLanguage().translateOption(v2, 'language'));
            }.bind(this));

            this.params.options.unshift('');

            this.translatedOptions = Nadlani.Utils.clone(this.getLanguage().translate('language', 'options') || {});
