...
Nadlani.define('views/stream/record/edit', 'views/record/base', function (Dep) {
    ...
    return Dep.extend({
        ...
        beforeSave: function () {
            Nadlani.Ui.notify(this.translate('posting', 'messages'));
            this.$post.addClass('disabled');
        },

        afterSave: function () {
            Nadlani.Ui.success(this.translate('Posted'));
            if (this.options.interactiveMode) {
                this.model.clear();
                this.model.set('targetType', 'self');
                this.model.set('type', 'Post');
