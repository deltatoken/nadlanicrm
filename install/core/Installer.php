...
use Nadlani\Core\Utils\File\Manager as FileManager;
...
class Installer
{
    ...
    public function __construct()
    {
        ...
        $this->app = new \Nadlani\Core\Application();
        ...
        $this->databaseHelper = new \Nadlani\Core\Utils\Database\Helper($this->getConfig());
    }
    ...
    protected function getPasswordHash()
    {
        if (!isset($this->passwordHash)) {
            ...
            $this->passwordHash = new \Nadlani\Core\Utils\PasswordHash($config);
        }
    ...
    {
        if (!$this->isAuth) {
            $auth = new \Nadlani\Core\Utils\Auth($this->app->getContainer());
            $auth->useNoAuth();
    ...
    {
         $systemRequirementManager = new \Nadlani\Core\Utils\SystemRequirements($this->app->getContainer());
