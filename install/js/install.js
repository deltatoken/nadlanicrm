...
InstallScript.prototype.getNadlaniPath = function(onlyPath) {
...
InstallScript.prototype.getModRewriteErrorMesssage = function() {
	...
	if (typeof(this.langs) !== 'undefined') {
		...
		message = message.replace("{ESPO_PATH}", this.getNadlaniPath(true)).replace("{API_PATH}", this.apiPath).replace("{API_PATH}", this.apiPath);
	}

	return message;
}

InstallScript.prototype.goToNadlani = function() {

	var location = this.getNadlaniPath();
