...
{
    /**
     * Get latest NadlaniCrm builded path
     *
     * @param  string $path
     *
     * @return string|null
    ...
    public static function getLatestBuildedPath($path)
    {
        ...
        while ($folderName = $buildDir->read()) {
            ...
            $pattern = '/^NadlaniCrm-([0-9]+)\.([0-9]+)(?:\.([0-9]+))?(?:-((a|alpha|b|beta|beta\.|pre|rc)[0-9]+)?)?$/';
    ...
    public static function versionCmp($a, $b)
    {
        ...
        $pattern = '/^NadlaniCrm-([0-9]+)\.([0-9]+)(?:\.([0-9]+))?(?:-((a|alpha|b|beta|pre|rc)[0-9]+)?)?$/';
