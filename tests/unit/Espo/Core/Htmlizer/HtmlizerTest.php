...
namespace tests\unit\Nadlani\Core\Htmlizer;
...
class HtmlizerTest extends \PHPUnit\Framework\TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->fileManager = $this->getMockBuilder('\\Nadlani\\Core\\Utils\\File\\Manager')->disableOriginalConstructor()->getMock();
        $this->fileManager
                    ->expects($this->any())
                    ->method('putContents')
        ...
        $this->dateTime = new \Nadlani\Core\Utils\DateTime('MM/DD/YYYY', 'hh:mm A', 'Europe/Kiev');
        $this->number = new \Nadlani\Core\Utils\NumberUtil('.', ',');

        $this->htmlizer = new \Nadlani\Core\Htmlizer\Htmlizer($this->fileManager, $this->dateTime, $this->number);
    }
